...
// Obtiene el estado real de una tarea: de asignaciones o del campo estado
...
export default function TareasPage() {
  ...
  useEffect(() => {
    ...
    const cargar = async () => {
      ...
      try {
        ...
        if (!cancelado) { setTareas(t); setSiembras(s); setTiposTarea(tt) }
      } catch {
  ...
  const recargar = async () => {
    try {
      const t = await tareasApi.listar()
      setTareas(t)
    } catch {
      ...
      <div style={{ display:'flex', flexWrap:'wrap', gap:'8px', alignItems:'center' }}>
        <div style={{ position:'relative', flex:1, minWidth:'200px' }}>
          ...
          <input
            ...
            placeholder="Buscar tarea..." className="input-field"
      ...
      {!loading && tareasFiltradas.length > 0 && (
        <div style={{ display:'flex', flexDirection:'column', gap:'8px' }}>
          {tareasFiltradas.map(tarea => {
            ...
            const vencida = tarea.fechaLimite && new Date(tarea.fechaLimite) < new Date() && estadoActual !== 'COMPLETADA'
            ...
            return (
              <div key={tarea.idTarea} className="card" style={{ display:'flex', alignItems:'center', gap:'16px', padding:'16px', borderLeft:`4px solid ${cfg.color}` }}>
                ...
                <div
                  ...
                  style={{
                    width:'24px', height:'24px', borderRadius:'6px', flexShrink:0, cursor:'pointer',
                    display:'flex', alignItems:'center', justifyContent:'center',
                    border:`2px solid ${cfg.color}`,
                    backgroundColor: estadoActual === 'COMPLETADA' ? cfg.color : 'transparent',
                    transition:'all 0.2s'
                  }}
                >
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:'8px', flexWrap:'wrap' }}>
                    ...
                    {vencida && (
                      <span style={{ padding:'2px 8px', borderRadius:'9999px', fontSize:'11px', fontWeight:600, backgroundColor:'var(--color-error-container)', color:'var(--color-error)' }}>
                        Vencida
                      </span>
                    )}
                  </div>
                  <p style={{
                    fontWeight:600, margin:'4px 0', fontSize:'0.9375rem',
                  ...
                  <div style={{ display:'flex', gap:'12px', flexWrap:'wrap' }}>
                    {tarea.nombreSiembra && (
                      <span style={{ fontSize:'0.75rem', color:'var(--color-on-surface-variant)', display:'flex', alignItems:'center', gap:'2px' }}>
                        <span className="material-symbols-outlined" style={{fontSize:'14px'}}>potted_plant</span>
                        {tarea.nombreSiembra}
                      </span>
                    )}
                    {tarea.fechaLimite && (
                ...
                <select
                  ...
                  style={{ padding:'4px 8px', borderRadius:'8px', border:'1px solid var(--color-outline-variant)', fontSize:'0.8rem', color:'var(--color-on-surface)', backgroundColor:'var(--color-surface-container-low)', cursor:'pointer' }}
                >
          ...
          <div className="card animate-slide-up" style={{ width:'100%', maxWidth:'480px' }}>
            ...
            <form onSubmit={handleGuardar} style={{ display:'flex', flexDirection:'column', gap:'16px' }}>
              ...
              <div>
                ...
                <input type="text" value={form.descripcion} onChange={e => setForm(p => ({...p, descripcion: e.target.value}))} placeholder="Describe la actividad..." className="input-field" />
              </div>
              <div>
              ...
              <div style={{ display:'flex', gap:'12px', paddingTop:'8px' }}>
                ...
                <button type="submit" disabled={saving} className="btn-primary" style={{flex:1}}>
                  {saving
                    ...
                    : <><span className="material-symbols-outlined" style={{fontSize:'18px'}}>save</span> Crear Tarea</>
                  }
                </button>
              </div>
            </form>
